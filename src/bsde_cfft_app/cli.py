"""MATH5030 console: CFFT-BSDE course tooling (lives outside the ``bsde_cfft_sv`` API package)."""
...
from bsde_cfft_app.benchmark_driver import BenchmarkConfig
from bsde_cfft_app.benchmark_driver import MarketParams
from bsde_cfft_app.benchmark_driver import parse_csv_floats
from bsde_cfft_app.benchmark_driver import parse_csv_ints
from bsde_cfft_app.benchmark_driver import parse_methods
from bsde_cfft_app.benchmark_driver import run_benchmark_comparison
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="bsde-cfft-sv",
        description="MATH5030 CFFT-BSDE course console (v0 core under implementation_version_0).",
    )
    parser.add_argument(
        "--menu",
        action="store_true",
    ...
    rep_mode.add_argument(
        ...
        action="store_true",
        help=(
            "Rerun the full bundled replication: numerical_results_quick.csv, "
            "numerical_results_surface_quick.csv under implementation_version_0/results, "
            "then paper-style PNGs (unless --skip-figures)"
        ),
    )
    rep_mode.add_argument(
        "--open-replication-report",
        action="store_true",
        help="Open bundled implementation_version_0/results/replication_report.html in the browser and exit (no solve)",
    )
    parser.add_argument(
        "--replication-report",
        type=Path,
    ...
    parser.add_argument(
        "--benchmark-output",
        type=str,
        default=str(RESULTS_DIR / "benchmark_comparison.csv"),
        help="Output CSV path for benchmark mode (default: implementation_version_0/results)",
    )
    parser.add_argument(
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if len(argv_list) == 0 and sys.stdin.isatty():
        from bsde_cfft_app.interactive_menu import run_replication_menu
    ...
    if args.run_replication:
        from bsde_cfft_app.replication_pipeline import run_full_replication
